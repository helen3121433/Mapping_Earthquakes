...
// add 3rd layer group for major earthquake data
...
let overlays = {
  ...
  "Tectonic plates group": tectonicPlateData,
...
d3.json("https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_week.geojson").then(function(data) {
  ...
  function majorEarthquakestyleInfo(feature) {
    return {
      opacity: 1,
      fillOpacity: 1,
      fillColor: majorEqgetColor(feature.properties.mag),
      color: "#000000",
      radius: majorEqgetRadius(feature.properties.mag),
      stroke: true,
      weight: 0.5
    };
  }
  ...
  function majorEqgetColor(magnitude) {
    if (magnitude > 6) {
      return "#4B0150";
    }
    if (magnitude > 5) {
  ...
  function majorEqgetRadius(magnitude){
    if (magnitude === 0) {
      return 1;
    }
    return magnitude * 4;
  ...
  L.geoJson(majorEarthquakedata, {
    pointToLayer: function(feature,latlng){
      ...
      // We set the style for each circleMarker using our styleInfo function.
    style: majorEarthquakestyleInfo,
     // We create a popup for each circleMarker to display the magnitude and location of the earthquake
     //  after the marker has been created and styled.
     onEachFeature: function(feature, layer) {
      layer.bindPopup("Magnitude: " + feature.properties.mag + "<br>Location: " + feature.properties.place);
